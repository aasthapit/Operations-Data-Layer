...
export function operatorsFor(kind) {
  return (OPS_BY_KIND[kind] || OPS_BY_KIND.text).map((id) => [id, OPS[id][0]]);
}
export function operatorLabel(op) {
  return OPS[op] ? OPS[op][0] : op;
}
// How many value inputs this operator needs, and what they should look like.
export function operatorInput(kind, op) {
  const spec = OPS[op];
  if (!spec) return { args: 0, type: null };
  const [, args, type] = spec;
...
function likeLiteral(value, wrap) {
...
// schema helpers
// --------------------------------------------------------------------------- //
export function tableOf(schema, name) {
  return (schema?.tables || []).find((t) => t.name === name) || null;
}

export function maxRowsOf(schema) {
  const n = Number(schema?.limits?.max_rows);
  return Number.isFinite(n) && n > 0 ? n : 500;
}

export function clampLimit(value, schema) {
...
export function canJoinClusters(schema, tableName) {
...
export function clusterContextOn(schema, state) {
...
export function availableColumns(schema, state) {
  const table = tableOf(schema, state.table);
  if (!table) return [];
  const out = table.columns.map((c) => ({
    id: c.name,
    label: c.name,
    column: c.name,
    type: c.type,
  ...
  for (const c of tableOf(schema, JOIN_TABLE).columns) {
...
export function defaultColumns(schema, tableName) {
...
export function defaultSorts(schema, tableName) {
...
export function clusterContextDefaults(schema, state) {
...
export function stateForTable(schema, tableName, base = {}) {
  return {
    v: STATE_VERSION,
    table: tableName,
...
export function defaultState(schema) {
...
export function normalizeState(raw) {
  ...
  return {
    ...
    columns: asArray(raw.columns).filter((c) => typeof c === "string"),
    filters: asArray(raw.filters).map((f, i) => ({
    ...
    sorts: asArray(raw.sorts)
      .filter((s) => s && typeof s.column === "string")
      .map((s) => ({ column: s.column, dir: s.dir === "desc" ? "desc" : "asc" })),
    group: {
      enabled: !!raw.group?.enabled,
      by: asArray(raw.group?.by).filter((c) => typeof c === "string"),
      aggs: asArray(raw.group?.aggs).map((a, i) => ({
...
export function pruneState(schema, state) {
...
export function aggNeedsColumn(fn) {
  return !!AGG_BY_ID[fn]?.[2];
}
export function aggNumericOnly(fn) {
...
// WHERE
// --------------------------------------------------------------------------- //
function conditionSql(col, filter, problems, index) {
  const { op } = filter;
  const e = col.expr;
  const needValue = (v) => {
    if (String(v ?? "").trim() === "") {
  ...
  const scalar = (v) => (col.kind === "number" ? numberOr(v) : lit(v));
  ...
  switch (op) {
    ...
    case "eq": case "ne": case "lt": case "lte": case "gt": case "gte": {
      ...
      const sign = { eq: "=", ne: "<>", lt: "<", lte: "<=", gt: ">", gte: ">=" }[op];
      return `${e} ${sign} ${v}`;
    }
...
// the generator
// --------------------------------------------------------------------------- //
export function buildSql(schema, state) {
  const problems = [];
  const table = tableOf(schema, state.table);
  ...
  if (grouping && state.group.by.length) {
    lines.push(`GROUP BY ${state.group.by.map((id) => byId.get(id).expr).join(", ")}`);
  }
...
export function sortableColumns(schema, state) {
  const columns = availableColumns(schema, state);
...
export function loadSaved() {
  try {
    ...
    return list
      .filter((q) => q && typeof q.name === "string" && q.state)
      .map((q) => ({ name: q.name, savedAt: q.savedAt || null, state: normalizeState(q.state) }))
      .filter((q) => q.state);        // an entry from an older shape is dropped
  } catch {
