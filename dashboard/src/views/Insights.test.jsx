...
const open = (section, at) => renderView(
...
describe("certificates", () => {
  it("lists what is expiring, worst first, with subject and issuer", async () => {
    open("certificates");
    const row = (await screen.findByText("router-certs-default")).closest("tr");
  ...
  it("opens the cluster a certificate is on", async () => {
    const user = userEvent.setup();
    open("certificates");
    const row = (await screen.findByText("checkout-tls")).closest("tr");
...
describe("pod issues", () => {
  it("lists the problem pods and summarises them by reason", async () => {
    ...
    const row = screen.getByText("checkout-api-7d9f8b6c4-2xk9p").closest("tr");
...
describe("quotas", () => {
  it("shows every resource in a quota against its hard limit", async () => {
    open("quotas");
    const row = (await screen.findByText("checkout-quota")).closest("tr");
...
describe("OLM operators", () => {
  it("shows version drift, unhealthy installs and pending upgrades per package", async () => {
    open("olm");
    const row = (await screen.findByText("elasticsearch-operator")).closest("tr");
  ...
  it("opens a package to show where each version is installed", async () => {
    ...
    const install = screen.getByText("elasticsearch-operator.v5.8.4").closest("tr");
...
describe("machine config pools", () => {
  it("lists the pools that are not settled, worst first", async () => {
    open("mcp");
    const row = (await screen.findByText("rendered-worker-6b2c1a")).closest("tr");
...
describe("storage", () => {
  ...
  it("puts the pending claims first and says what is not mounted", async () => {
    open("storage");
    const row = (await screen.findByText("checkout-data")).closest("tr");
...
describe("events", () => {
  it("lists the warning events and summarises them by reason", async () => {
    ...
    const row = screen.getByText("Pod/prometheus-k8s-1").closest("tr");
...
describe("config references", () => {
  it("shows which workloads reference a secret, and how", async () => {
    open("references");
    const row = (await screen.findByText("checkout-db")).closest("tr");
...
describe("cluster admins", () => {
  it("names every subject holding cluster-admin and where", async () => {
    open("access");
    const row = (await screen.findByText("sre-oncall")).closest("tr");
  ...
  it("names the namespace a service account lives in", async () => {
    open("access");
    const row = (await screen.findByText("pipeline")).closest("tr");
