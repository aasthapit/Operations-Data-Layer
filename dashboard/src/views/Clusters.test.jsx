...
describe("the table", () => {
  ...
  it("shows where a cluster is upgrading to", async () => {
    open();
    const row = (await screen.findByText("ocp-stage-iad-01")).closest("tr");
...
describe("the filters", () => {
  it("builds each dropdown from the whole fleet, not from what is on screen", async () => {
    ...
    expect([...screen.getByLabelText("Hub").options].map((o) => o.value))
      .toEqual(["", "hub-east", "hub-west"]);
    expect([...screen.getByLabelText("OCP version").options].map((o) => o.value))
