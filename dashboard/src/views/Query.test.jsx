...
describe("opening the page", () => {
  ...
  it("offers every table the running API has, with its row count", async () => {
    open();
    const tables = await screen.findByLabelText("Data set");
...
describe("the builder", () => {
  ...
  it("offers only the operators the column's type can answer", async () => {
    ...
    expect([...screen.getByLabelText("Filter operator").options].map((o) => o.value))
  ...
  it("offers only numeric columns to an aggregate that needs one", async () => {
    ...
    expect([...screen.getByLabelText("Aggregate column").options].map((o) => o.value))
...
describe("asking in English", () => {
  ...
  it("shows the last query it tried when both attempts failed", async () => {
    const user = userEvent.setup();
    const error = Object.assign(new Error("both attempts failed"), { status: 422 });
    error.detail = { error: "both attempts failed", sql: "SELECT * FROM pods" };
  ...
  it("runs a ready-made trend query", async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText("SQL").value).toContain("date_trunc"));
  });
  ...
  it("runs an example the schema shipped", async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText("SQL").value)
