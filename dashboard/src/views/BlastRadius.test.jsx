...
describe("the form", () => {
  ...
  it("offers the versions, operators and packages the fleet actually has", async () => {
    open();
    const ocp = await screen.findByLabelText("OCP version");
    expect([...ocp.options].map((o) => o.value)).toEqual(["", "4.16.7", "4.16.4", "4.15.22"]);
    expect([...screen.getByLabelText("Cluster operator").options].map((o) => o.value))
      .toEqual(["", "ingress", "network", "authentication"]);
    expect([...screen.getByLabelText("OLM operator").options].map((o) => o.value))
  ...
  it("keeps the version pickers shut until their operator is chosen", async () => {
    ...
    const versions = screen.getByLabelText("Operator version");
  ...
  it("offers the OLM versions of the package that was chosen", async () => {
    ...
    expect([...screen.getByLabelText("OLM version").options].map((o) => o.value))
      .toEqual(["", "5.8.6", "5.8.4"]);
  });
...
describe("the report", () => {
  ...
  it("counts what the change would touch", async () => {
    openReport();
    const stats = (await screen.findByText("Clusters impacted")).closest(".stats");
  ...
  it("lists the clusters with why each one matched", async () => {
    ...
    const clusters = container.querySelector("#blast-clusters, .card.flush");
    const row = within(clusters).getByText("ocp-stage-iad-01").closest("tr");
  ...
  it("shows the spread by environment, by hub and over the platform", async () => {
    openReport();
    const spread = (await screen.findByText("By environment")).closest(".card");
    expect(within(spread).getByText("prod").closest("div")).toHaveTextContent("prod 1");
    expect(within(spread).getByText("stage").closest("div")).toHaveTextContent("stage 1");
    expect(within(spread).getByText("By hub")).toBeInTheDocument();
    expect(within(spread).getByText("hub-east").closest("div")).toHaveTextContent("hub-east 2");
  ...
  it("opens a cluster from the impacted list", async () => {
    ...
    const { container } = openReport();
    await screen.findByText("Impacted clusters");
    const clusters = container.querySelector(".card.flush");
