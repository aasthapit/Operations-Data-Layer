...
const open = (at = "/dashboards") => renderView(
  ({ route, nav }) => <Dashboards route={route} nav={nav} />, { at });
...
describe("the list", () => {
  ...
  it("shows a variable slot in a title as the variable rather than as a gap", async () => {
    open();
    const card = (await screen.findByText(/Hub review/)).closest(".db-card");
  ...
  it("says panel rather than panels for one, and says when there is no description",
    async () => {
      open();
      const card = (await screen.findByText("Capacity watch")).closest(".db-card");
