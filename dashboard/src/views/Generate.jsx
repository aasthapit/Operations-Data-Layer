...
const targetOf = (item) => item.args?.id || (PARTIAL_ID.exec(item.argsText || "") || [])[1] || "";

const inFlight = (item, name) =>
...
function usePanelRun(definition, params) {
  const [run, setRun] = useState({ results: {}, variables: {}, params: {}, error: null, loading: false });
  const [nonce, setNonce] = useState(0);
  const latest = useRef({ definition, params });
  latest.current = { definition, params };
  const abort = useRef(null);
  const asked = useRef({});           // panel id -> the query it was last asked
...
// the page
// --------------------------------------------------------------------------- //
export default function Generate({ route, nav }) {
  ...
  const written = { title: state.dashboard?.title || "", description: state.dashboard?.description || "" };
  const run = usePanelRun(definition, state.params || {});
  const params = run.params && Object.keys(run.params).length ? run.params : (state.params || {});
  ...
  const pendingPanels = transcript.filter((i) => inFlight(i, "add_panel"));
  const updating = new Set(transcript.filter((i) => inFlight(i, "update_panel")).map(targetOf));
  ...
  const save = async (id) => {
    ...
    try {
      ...
      setProblem(fieldErrors(e)[0]?.message || String(e?.message || e));
      setSaving(false);
    }
  };
...
// the conversation
// --------------------------------------------------------------------------- //
function Transcript({ items, running, result }) {
...
function Cost({ result }) {
  const parts = [];
  if (result.turns != null) parts.push(`${result.turns} ${result.turns === 1 ? "turn" : "turns"}`);
  if (result.tool_calls != null) parts.push(`${result.tool_calls} tool calls`);
  if (result.elapsed_ms != null) parts.push(`${(result.elapsed_ms / 1000).toFixed(1)} s`);
  if (!parts.length) return null;
  return <div className="gen-cost">{parts.join(" · ")}</div>;
}

function Intro({ onPick, disabled }) {
  return (
...
function Composer({ prefill, running, disabled, onSend, onStop }) {
...
// waiting.
function PendingPanel({ title }) {
  return (
    <section className="db-panel card gen-pending" style={{ "--w": 6, "--h": 2 }}
...
function Availability({ info, fixture, route }) {
