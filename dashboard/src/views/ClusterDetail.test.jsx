...
const open = (tab) => renderView(({ nav }) => <ClusterDetail name={CLUSTER_NAME} tab={tab}
...
describe("the overview tab", () => {
  ...
  it("reads capacity as used against allocatable, with what is requested marked", async () => {
    open();
    const capacity = (await screen.findByRole("heading", { name: /Capacity/ })).closest(".card");
...
describe("the namespaces tab", () => {
  ...
  it("shows ownership only on the application table", async () => {
    open("namespaces");
    const apps = (await screen.findByText("Applications (1)")).closest(".card");
    ...
    const platform = screen.getByText("OpenShift platform namespaces (1)").closest(".card");
  ...
  it("reads a namespace's pods, restarts, usage and resource counts", async () => {
    open("namespaces");
    const row = (await screen.findByText("checkout-prod")).closest("tr");
...
describe("the nodes tab", () => {
  it("draws a row per node with its state, usage and identity", async () => {
    open("nodes");
    const row = (await screen.findByText("ip-10-4-1-21.ec2.internal")).closest("tr");
  ...
  it("says a node is cordoned and under pressure", async () => {
    open("nodes");
    const row = (await screen.findByText("ip-10-4-2-44.ec2.internal")).closest("tr");
...
describe("the issues tab", () => {
  it("lists the problem pods with what is wrong", async () => {
    open("issues");
    const row = (await screen.findByText("checkout-api-7d9f8b6c4-2xk9p")).closest("tr");
  ...
  it("says a pending pod has no node yet rather than leaving the cell blank", async () => {
    open("issues");
    const row = (await screen.findByText("prometheus-k8s-1")).closest("tr");
  ...
  it("lists the recent warning events", async () => {
    open("issues");
    const row = (await screen.findByText("Pod/prometheus-k8s-1")).closest("tr");
...
describe("the operators tab", () => {
  it("reads each operator's state out of its conditions", async () => {
    ...
    const state = async (name) =>
      within((await screen.findByText(name)).closest("tr")).getAllByRole("cell")[2].textContent;
  ...
  it("marks the operators an upgrade cannot proceed without", async () => {
    open("operators");
    const row = (await screen.findByText("authentication")).closest("tr");
  ...
  it("says an unavailable operator is unavailable", async () => {
    ...
    open("operators");
    const row = (await screen.findByText("dns")).closest("tr");
...
describe("summarize", () => {
  const line = (key, summary) => summarize({ key, summary });
