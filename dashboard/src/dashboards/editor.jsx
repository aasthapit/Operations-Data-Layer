...
// a panel
// --------------------------------------------------------------------------- //
export function PanelDrawer({ panel, definition, params, result, errors, onApply, onClose }) {
  const [draft, setDraft] = useState(panel);
  const [preview, setPreview] = useState(result && !result.error ? result : null);
...
// the variables
// --------------------------------------------------------------------------- //
export function VariablesDrawer({ definition, errors, onApply, onClose }) {
  const [list, setList] = useState(definition.variables || []);

  const set = (i, fields) => setList((l) => l.map((v, j) => (j === i ? normalizeVariable({ ...v, ...fields }, j) : v)));
      ...
      {list.map((v, i) => (
        <div className="db-var-edit" key={i /* eslint-disable-line react/no-array-index-key */}>
          <div className="db-drawer-row">
            ...
            <Field label="Type">
              <select value={v.type} onChange={(e) => set(i, { type: e.target.value })}>
          ...
          <div className="db-drawer-row">
            <Field label="Default" error={errorAt(errors, `variables.${i}.default`)}>
              <input type={v.type === "number" ? "number" : "text"}
                value={hasValue(v.default) ? v.default : ""}
