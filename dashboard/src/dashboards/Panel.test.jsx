...
const draw = (props = {}) => render(
  <Panel panel={PANEL} definition={DEFINITION} params={{ hub: "hub-east" }} {...props} />);
...
describe("the header", () => {
  ...
  it("offers Open in Query always, and Edit and Remove only while editing", async () => {
    ...
    rerender(<Panel panel={PANEL} definition={DEFINITION} params={{ hub: "hub-east" }}
      result={TABLE_RESULT} editing onEdit={vi.fn()} onRemove={vi.fn()} />);
    await user.click(screen.getByRole("button", { name: "⋯" }));
...
describe("the body", () => {
  ...
  it("draws a chart when the columns can carry the one the panel asked for", () => {
    const charted = { ...PANEL, chart: { type: "bars", x: "overall_status", series: "",
  ...
  it("gives a tall panel a filter row and a short one none", () => {
    const { rerender } = render(<Panel panel={{ ...PANEL, h: 3 }} definition={DEFINITION}
      params={{ hub: "hub-east" }} result={TABLE_RESULT} />);
