...
function kindFromValues(rows, i) {
  let seen = 0;
  let numbers = 0;
  let dates = 0;
  let scalars = 0;
...
export function inferFields(columns, columnTypes, rows) {
...
const byName = (fields, name) => fields.find((f) => f.name === name) || null;
const timeFields = (fields) => fields.filter((f) => f.kind === "time");
const numberFields = (fields) => fields.filter((f) => f.kind === "number");
const measureFields = (fields) => numberFields(fields).filter((f) => !f.idLike);
export const categoryFields = (fields) => fields.filter((f) => f.kind === "cat");
...
function forcedSpec(type, fields, rows) {
  const measures = measureFields(fields);
...
export function normalizeChart(raw) {
  return {
    type: ["auto", "line", "bars", "none"].includes(raw?.type) ? raw.type : "auto",
    x: typeof raw?.x === "string" ? raw.x : "",
    series: typeof raw?.series === "string" ? raw.series : null,
    y: Array.isArray(raw?.y) ? raw.y.filter((n) => typeof n === "string") : [],
    stack: !!raw?.stack,
  };
}
...
export function resolveSpec(fields, rows, choice) {
...
function formatTimeTick(t, unit) {
  const d = new Date(t);
...
function formatTimeFull(t, unit) {
  const d = new Date(t);
...
const colorAt = (i, total) => (total <= 1 ? "var(--chart-line)" : SERIES_COLORS[i % SERIES_COLORS.length]);

// --------------------------------------------------------------------------- //
// models
// --------------------------------------------------------------------------- //
const keyOf = (v) => (v == null ? "—" : typeof v === "object" ? JSON.stringify(v) : String(v));
...
function nearestPoint(points, t, tol) {
...
function buildLineModel(fields, rows, spec) {
  const x = byName(fields, spec.x);
  const seriesField = spec.series ? byName(fields, spec.series) : null;
  const measures = spec.y.map((n) => byName(fields, n)).filter(Boolean);
  ...
  const bucket = (key, measure) => {
    let g = groups.get(key);
    if (!g) { g = { key, measure, points: new Map() }; groups.set(key, g); }
    return g;
  };
  for (const row of rows) {
    const t = parseTime(row[x.index]);
    if (t == null) continue;
    if (seriesField) {
      const g = bucket(keyOf(row[seriesField.index]), measures[0]);
      const v = row[measures[0].index];
      g.points.set(t, typeof v === "number" ? v : null);
    } else {
  ...
  if (stacked) {
    ...
    const running = new Map(stamps.map((t) => [t, 0]));
    for (const s of series) {
      let carried = 0;
      s.band = stamps.map((t) => {
...
function buildBarModel(fields, rows, spec) {
  const x = byName(fields, spec.x);
  const measures = spec.y.map((n) => byName(fields, n)).filter(Boolean).slice(0, GROUPED_CAP);
  if (!x || !measures.length) return null;
  ...
  const items = shown.map((row, i) => ({
    key: `${i}:${keyOf(row[x.index])}`,
    label: keyOf(row[x.index]),
    values: measures.map((m) => (typeof row[m.index] === "number" ? row[m.index] : null)),
  }));
  const flat = items.flatMap((it) => it.values).filter((v) => v != null);
...
// line
// --------------------------------------------------------------------------- //
function LineChart({ model, height }) {
  const wrap = useRef(null);
  const width = useWidth(wrap);
  const [hover, setHover] = useState(null);          // an index into model.stamps
  ...
  const hovered = hoverT == null ? [] : model.series.map((s) => {
    ...
    return { series: s, point: p && p.v != null ? p : null, top: p && p.v != null ? (model.stacked ? p.top : p.v) : null };
  });
...
// bars
// --------------------------------------------------------------------------- //
function BarChart({ model, height }) {
  const wrap = useRef(null);
  const width = useWidth(wrap);
  const [hover, setHover] = useState(null);          // "item:series"
  ...
  const tip = !item ? null : {
    x: horizontal
      ? Math.min(vx(Math.max(...item.values.filter((v) => v != null), zero)), width - 4)
...
function lineAria(model, tableBelow) {
...
function barAria(model, tableBelow) {
...
export default function Chart({
  ...
  const model = useMemo(() => {
    if (!spec) return null;
    const built = spec.type === "line"
      ? buildLineModel(fields, rows || [], spec)
      : buildBarModel(fields, rows || [], spec);
    if (!built) return null;
    built.ariaLabel = spec.type === "line"
      ? lineAria(built, tableBelow)
      : barAria(built, tableBelow);
    return built;
  ...
  return (
    <div className="chart">
      <Legend series={model.series} mark={spec.type === "line" && !model.stacked ? "line" : "box"} />
      {spec.type === "line"
        ? <LineChart model={model} height={height} />
        : <BarChart model={model} height={height} />}
