...
# dashboard, and the MCP server. Procfile.local is the no-Docker variant.
redis:     ${COMPOSE:-docker compose} up --no-log-prefix redis
collector: ${COMPOSE:-docker compose} up --no-log-prefix --no-deps api
api:       sh -c 'cd data-layer && COLLECTOR_ENABLED=false REDIS_URL=${REDIS_URL:-redis://localhost:${ODL_REDIS_PORT:-16379}/0} ODL_MANIFEST=config/ocp-api-manifest.yaml ODL_CONFIG=config/hubs.yaml exec .venv/bin/uvicorn app.main:app --reload --host 127.0.0.1 --port ${DEV_API_PORT:-18002}'
